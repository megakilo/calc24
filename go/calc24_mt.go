...
func (c *Calc24) Calc(nums []int, output chan string) {
	target := float64(24)
	for _, expr := range c.expressions {
		if expr.eval(nums) == target {
			output <- fmt.Sprintf("%v -> %s", nums, expr.toString(nums))
		}
	}
	output <- fmt.Sprintf("%v -> No Solution", nums)
}
